...
prompt_pure_seconds_to_human_time() {
...
prompt_pure_git_time_since_commit() {
  if [[ $(git log -1 2>&1 > /dev/null | grep -c "^fatal: Not a git repository (or any of the parent directories): .git") == 0 ]]; then
    # Get the last commit.
    last_commit=$(git log --pretty=format:'%at' -1 2> /dev/null)
    now=$(date +%s)
    seconds_since_last_commit=$((now - last_commit))

    echo $(prompt_pure_seconds_to_human_time $seconds_since_last_commit)
  fi
}
...
prompt_pure_preprompt_render() {
	...
	# local preprompt="%F{green}[%*] %F{blue}%~%f"
  # Uncomment the following line if you don't want the time.
	local preprompt="%F{blue}%~%f"
	# git info
	preprompt+="%F{$git_color}${vcs_info_msg_0_} %F{red}${prompt_pure_git_dirty}%f"
	# git pull/push arrows
	preprompt+="%F{cyan}${prompt_pure_git_arrows}%f"
  # git last time since commit
  preprompt+=" %F{green}$(prompt_pure_git_time_since_commit)%f"
	# username and machine if applicable
	preprompt+=$prompt_pure_username
	# execution time
  if [ ! -z "${prompt_pure_cmd_exec_time}" ]; then
    preprompt+=" %F{cyan}●%F{yellow}${prompt_pure_cmd_exec_time}%f"
  fi
	...
	if [[ "$1" == "precmd" ]]; then
    # No newline after every command.
		print -P "${preprompt}"
